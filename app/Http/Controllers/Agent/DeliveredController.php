    ...
    public function delivered(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = DB::table('dr')
                ...
                ->whereRaw('dr_no in (select dr_no from dr_items where status = "DELIVERED")');
        ...
        return view('agent.delivered.index')
            ->with('active', 'delivered')
            ->with('title', 'DELIVERED');
    }

    public function delivered_items(Request $request, $dr)
    {
        if ($request->ajax()) {
    ...
    public function confirm(Request $request, $dr, $date)
    {
        if ($request->ajax()) {
            ...
            $dr_items_confirmed = DB::table('dr_items')
                ...
                ->where('original_status', 'DELIVERED')
                ->get();
            ...
            // audit trail
            $audit = new Audit();
            $audit->prev_status = 'CONFIRMED';
            $audit->new_status = 'DELIVERED';
